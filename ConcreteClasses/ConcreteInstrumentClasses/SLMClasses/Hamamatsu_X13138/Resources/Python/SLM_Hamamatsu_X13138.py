# from cffi import FFI
# from PIL import Image
# import numpy as np
from ctypes import *
import matplotlib.pyplot as plt
from slm_base import SLM

    
class HamaSLM_13138(SLM):
    def __init__(self, monitor, modulation_depth) -> None:
        #pixelpitch(0: 20um 1: 1.25um)
        self.pitch = 1
        #LCOS pixel size
        self.x = 1272
        self.y = 1024
        #LCOS-SML monitor number setting
        self.monitorNo = monitor # 2
        self.windowNo = 0
        self.xShift = 0
        self.yShift = 0
        #pixel number
        self.array_size = self.x * self.y
        self.modulation_depth = modulation_depth #for 810 --> 214 #256

        # make the 8bit unsigned integer array type
        self.FARRAY = c_uint8 * self.array_size
        self.farray = self.FARRAY(0)

        
        self.Lcoslib = windll.LoadLibrary(r"C:\Users\quantum\Desktop\Panagiotis\Heisingberg-Experiment\ising-machine-main\ising_machine\slm\Image_Control.dll")

                    

        #Select LCOS window
        Window_Settings = self.Lcoslib.Window_Settings
        Window_Settings.argtypes = [c_int, c_int, c_int, c_int]
        Window_Settings.restype = c_int
        Window_Settings(self.monitorNo, self.windowNo, self.xShift, self.yShift)

                    
    def upload_phase(self, array):
        """uploads uint8 phase to the SLM Display

        Returns
        -------
        None
        """
        n = array.shape[0]
        k = array.shape[1]
#         array = array % self.modulation_depth
  


        outputPattern = array
        # perhaps this one should be defined as integer:
        # outputPattern = np.asarray(array, dtype=int)
        
        
#         # Flatten the array, modulate the values, and convert to uint8
#         flat_array = (array.flatten() % self.modulation_depth).astype(np.uint8)

#         # Copy the NumPy array directly into the ctypes array
#         ctypes.memmove(self.farray, flat_array.ctypes.data, flat_array.nbytes)

        for i in range(n):
            for j in range(k):
                self.farray[i*k + j] = c_uint8(int(outputPattern[i][j] % self.modulation_depth))

        
        # Show pattern
        Window_Array_to_Display = self.Lcoslib.Window_Array_to_Display
        Window_Array_to_Display.argtypes = [c_void_p, c_int, c_int, c_int, c_int]
        Window_Array_to_Display.restype = c_int
        Window_Array_to_Display(self.farray, self.x, self.y, self.windowNo, self.array_size)
        
        # # close the window
        # Window_Term = Lcoslib.Window_Term
        # Window_Term.argtyes = [c_int]
        # Window_Term.restype = c_int
        # Window_Term(self.windowNo)


        return 0
    
    def get_Size(self):
        """Returns the Size of the SLM Display.

        Returns
        -------
        tuple
            (x_size, y_size) of SLM screen.
        """
        return self.x, self.y
    
    def __del__(self):
        """ Deletes the Object

        Returns
        -------
        None
        """
        print('SLM Object Deleted')
        


        